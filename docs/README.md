...
- MCP: tool contracts live in [crates/mcp-server/src/tools.rs](../crates/mcp-server/src/tools.rs) (includes `repo_onboarding_pack` for one-call onboarding, `grep_context` for regex context reads, `batch` for one-call orchestration with optional `$ref` dependencies, and cursor pagination for large outputs)
