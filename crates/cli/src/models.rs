...
pub fn doctor(model_dir: &Path) -> DoctorReport {
    let mut report = DoctorReport {
        model_dir: model_dir.display().to_string(),
        profile: std::env::var("CONTEXT_FINDER_PROFILE").unwrap_or_else(|_| "quality".to_string()),
        embedding_mode: std::env::var("CONTEXT_FINDER_EMBEDDING_MODE")
            .unwrap_or_else(|_| "fast".to_string()),
        embedding_model: std::env::var("CONTEXT_FINDER_EMBEDDING_MODEL")
            .unwrap_or_else(|_| "bge-small".to_string()),
        allow_cpu_fallback: std::env::var("CONTEXT_FINDER_ALLOW_CPU")
